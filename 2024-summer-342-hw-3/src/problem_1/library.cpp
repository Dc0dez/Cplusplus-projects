...
bool Library::borrow_book(const std::string &book_name, Book &borrowed_book, int &time_of_operation_in_sec) {
    ...
    if (found) {
        /*
         * I got help from Peng narrowing down the logic error in this function
         * Task 1.3:
         * The logic error in this code is that when a book is borrowed it is not being removed from the front desk shelf
         * To ensure the book is removed from the shelf when it is borrowed, I added a line of code removing it on line 17
         */
        time_of_operation_in_sec = front_desk_shelf.get_capacity();  // each book in the front desk shelf takes 1 sec to search
        front_desk_shelf.remove(book_name);
        return true;
    }
